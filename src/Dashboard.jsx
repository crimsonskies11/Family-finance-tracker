...
import { fetchYahooBatch, fetchTickerInfo, getMarketStatus, formatAge } from "./api.js";
...
function MarketStatusBadge() {
  ...
  const cfg = {
    open:   { color: C.green,  label: "Market Open" },
    pre:    { color: C.gold,   label: "Pre-Market" },
    after:  { color: C.orange, label: "After-Hours" },
    closed: { color: C.muted,  label: "Market Closed" },
  }[status];
  return (
...
function LiveClock() {
  ...
  useEffect(() => {
    const tick = () => setTime(
      new Date().toLocaleTimeString("en-US", { timeZone: "America/New_York", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false })
    );
    tick();
  ...
  return (
    <span style={{ fontSize: "0.6rem", color: C.dim, fontFamily: "monospace" }}>{time} ET</span>
  );
}
...
function RefreshButton({ stocks, onRefreshComplete, autoInterval, onAutoIntervalChange }) {
  ...
  // Tick the "X ago" label every 15 s
  useEffect(() => {
  ...
  const doRefresh = useCallback(async () => {
    ...
    const tickers = tickerStocks.map((s) => s.ticker);
    const updates = [];
    const errs = [];

    try {
      // Single batch request to Yahoo Finance
      const priceMap = await fetchYahooBatch(tickers);

      for (const s of tickerStocks) {
        const d = priceMap[s.ticker];
        if (d) {
          updates.push({ id: s.id, price: d.price, change: d.change, source: "yahoo", marketState: d.marketState, updatedAt: d.updatedAt });
        } else {
          errs.push(s.ticker); // collect misses for parallel fallback
        }
      }

      // Parallel Claude fallback for any Yahoo misses
      if (errs.length) {
        const fallbacks = await Promise.allSettled(
          errs.map(async (ticker) => {
        ...
        fallbacks.forEach((r, i) => {
          if (r.status === "fulfilled") {
            ...
            errs.splice(errs.indexOf(tickers[i]), 1);
          }
        });
      }
    } catch {
      // Yahoo batch completely failed — fall back to parallel Claude calls for all
      const fallbacks = await Promise.allSettled(
        tickerStocks.map(async (s) => {
      ...
      fallbacks.forEach((r, i) => {
        ...
        else errs.push(tickerStocks[i].ticker);
      });
    }
    ...
    const now = Date.now();
    setLastTs(now);
    lsSet("ffd_lastRefreshed", now);
    setAge(formatAge(now));
        ...
        {state === "loading" ? (
          ...
          : <>
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
              ...
              <path d="M6 0.5L7.5 2L6 3.5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
...
function AddModal({ onAdd, onClose }) {
  ...
  const handleFetch = async () => {
    ...
    try {
      const data = await fetchTickerInfo(sym);
      setFetched(data);
    } catch {
      setError(`Could not find "${sym}". Try manual entry instead.`);
    }
    setLoading(false);
  };
  ...
  const confirmTicker = () => {
    ...
    onAdd({
      id: ++idCounter, ticker: ticker.trim().toUpperCase(), name: fetched.name,
      shares: parseFloat(shares), price: fetched.price, change: fetched.change ?? 0,
      type: fetched.type, color: pickColor(), isPrimary: false, source: fetched.source ?? "yahoo",
    });
    onClose();
  };
  ...
  const confirmManual = () => {
    ...
    onAdd({
      id: ++idCounter, ticker: manualName.replace(/\s+/g, "").slice(0, 6).toUpperCase(),
      name: manualName, shares: 1, price: parseFloat(manualValue), change: 0,
      type: manualType, color: pickColor(), isPrimary: false, isManual: true, source: "manual",
    });
    onClose();
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#07090fee", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "1.75rem", width: 400, maxWidth: "100%" }}>
        ...
        {mode === "ticker" && (
          <div>
            <Lbl>Stock / ETF / Mutual Fund / Crypto Symbol</Lbl>
            <div style={{ display: "flex", gap: 6, marginBottom: "0.75rem" }}>
              <Inp value={ticker} onChange={(e) => { setTicker(e.target.value.toUpperCase()); setError(""); setFetched(null); }} placeholder="e.g. NVDA, VINIX, BTC" style={{ flex: 1 }} />
              <button onClick={handleFetch} disabled={loading || !ticker.trim()} style={{ background: loading ? C.muted : C.accent, border: "none", color: "#07090f", borderRadius: 6, padding: "0 0.9rem", cursor: loading ? "not-allowed" : "pointer", fontWeight: 700, fontSize: "0.75rem", whiteSpace: "nowrap" }}>
                {loading ? "…" : "Look Up"}
              </button>
            </div>
            ...
            {error && (
              <div style={{ background: `${C.red}12`, border: `1px solid ${C.red}33`, borderRadius: 7, padding: "0.65rem 0.75rem", marginBottom: "0.75rem" }}>
                <div style={{ color: C.red, fontSize: "0.7rem", marginBottom: "0.3rem", fontWeight: 600 }}>Lookup failed</div>
                <div style={{ color: C.dim, fontSize: "0.68rem" }}>{error}</div>
                <button onClick={() => setMode("manual")} style={{ marginTop: "0.5rem", background: "transparent", border: `1px solid ${C.border}`, color: C.dim, borderRadius: 5, padding: "0.3rem 0.65rem", cursor: "pointer", fontSize: "0.68rem" }}>Switch to Manual Entry →</button>
              </div>
            )}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    ...
                    <div style={{ marginTop: 4, display: "flex", gap: 4 }}>
                      <Tag color={fetched.type === "crypto" ? C.gold : fetched.type === "mutualfund" ? C.green : C.accent}>
                        {fetched.type === "mutualfund" ? "FUND" : fetched.type?.toUpperCase()}
                      </Tag>
            ...
            {fetched && (
              <>
                ...
                <button onClick={confirmTicker} disabled={!shares} style={{ width: "100%", background: C.accent, border: "none", color: "#07090f", borderRadius: 7, padding: "0.6rem", fontWeight: 700, fontSize: "0.8rem", cursor: shares ? "pointer" : "not-allowed", opacity: shares ? 1 : 0.5 }}>
                  Add to Dashboard
                </button>
              </>
            )}
        ...
        {mode === "manual" && (
          <div>
            <div style={{ display: "flex", gap: 4, marginBottom: "1rem" }}>
              {["stock", "etf", "crypto", "mutualfund"].map((t) => (
                <Pill key={t} active={manualType === t} onClick={() => setManualType(t)}>
                  {t === "mutualfund" ? "FUND" : t.toUpperCase()}
                </Pill>
              ))}
            </div>
            <Lbl>Name / Label</Lbl>
            ...
            <button onClick={confirmManual} disabled={!manualName || !manualValue} style={{ width: "100%", background: C.gold, border: "none", color: "#07090f", borderRadius: 7, padding: "0.6rem", fontWeight: 700, fontSize: "0.8rem", cursor: "pointer", opacity: manualName && manualValue ? 1 : 0.5 }}>
              Add to Dashboard
            </button>
          </div>
        )}
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, padding: "1.25rem", fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.25rem", flexWrap: "wrap", gap: "0.75rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.45rem", flexWrap: "wrap", alignItems: "center" }}>
          ...
          />
          <button onClick={() => setShowModal(true)} style={{ background: C.accent, border: "none", color: "#07090f", borderRadius: 7, padding: "0.4rem 0.85rem", fontWeight: 700, fontSize: "0.72rem", cursor: "pointer" }}>
            + Add
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 3, marginBottom: "1.1rem", background: C.card, padding: "0.25rem", borderRadius: 8, border: `1px solid ${C.border}`, width: "fit-content" }}>
        {["overview", "investments", "milestones", "adjust"].map((t) => (
            ...
            <Card key={s.id}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.85rem" }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3, flexWrap: "wrap" }}>
                    ...
                    <Tag color={s.type === "crypto" ? C.gold : s.type === "etf" ? C.purple : s.type === "mutualfund" ? C.green : C.accent}>
                      {s.type === "mutualfund" ? "FUND" : s.type?.toUpperCase()}
                    </Tag>
                    <SourceBadge source={s.source} />
                  </div>
                ...
                {!s.isPrimary && (
                  <button onClick={() => removeInv(s.id)} style={{ background: "none", border: "none", color: C.red, cursor: "pointer", fontSize: "0.85rem", opacity: 0.65, padding: 0 }}>✕</button>
                )}
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem", marginBottom: "0.7rem" }}>
                <div>
                  ...
                  <input type="number" value={s.isManual ? s.price : s.shares}
                    onChange={(e) => s.isManual ? updateManualVal(s.id, e.target.value) : updateShares(s.id, e.target.value)}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(270px, 1fr))", gap: "0.85rem" }}>
          ...
          <Card>
            ...
            {[250000, 500000, 750000, 1000000, 1500000, 2000000].map((m) => {
              ...
              const close = !done && netWorth >= m * 0.8;
              return (
          ...
          <Card>
            ...
            {stocks.map((s) => (
              <div key={s.id} style={{ marginBottom: "0.85rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ color: s.color, fontFamily: "monospace", fontSize: "0.73rem", fontWeight: 700 }}>
                    {s.ticker} <span style={{ color: C.dim, fontWeight: 400, fontSize: "0.65rem" }}>{s.name}</span>
                  </span>
                  <span style={{ fontSize: "0.68rem", color: C.dim }}>{fc(s.shares * s.price)}</span>
                </div>
                <div style={{ display: "flex", gap: 5 }}>
                  <button onClick={() => s.isManual ? updateManualVal(s.id, Math.max(0, s.price - 100)) : updateShares(s.id, Math.max(0, s.shares - 1))}
                    style={{ background: C.border, border: "none", color: C.text, borderRadius: 4, width: 26, height: 26, cursor: "pointer", flexShrink: 0 }}>−</button>
                  <input type="number" value={s.isManual ? s.price : s.shares}
                    onChange={(e) => s.isManual ? updateManualVal(s.id, e.target.value) : updateShares(s.id, e.target.value)}
                  ...
                  <button onClick={() => s.isManual ? updateManualVal(s.id, s.price + 100) : updateShares(s.id, s.shares + 1)}
                    style={{ background: C.accent, border: "none", color: "#07090f", borderRadius: 4, width: 26, height: 26, cursor: "pointer", fontWeight: 700, flexShrink: 0 }}>+</button>
                </div>
              </div>
            ))}
            <button onClick={() => setShowModal(true)} style={{ width: "100%", background: "transparent", border: `1px dashed ${C.border}`, color: C.muted, borderRadius: 7, padding: "0.5rem", cursor: "pointer", fontSize: "0.73rem", marginTop: "0.5rem" }}>
              + Add New Investment
            </button>
          </Card>
          <Card>
